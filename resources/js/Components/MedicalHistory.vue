<template>
    <div class="max-w-4xl mx-auto p-6 bg-white shadow-md rounded-lg">
        ...
        <div class="mb-4">
            ...
            <p class="text-gray-700">{{ medicalHistory.medications ? 'Yes' : 'No' }}</p>
        </div>

        <div v-if="medicalHistory.medications" class="mb-4">
            <h2 class="text-lg font-semibold">Medication Details</h2>
            <p class="text-gray-700">{{ medicalHistory.medications || 'N/A' }}</p>
        </div>

        <div class="mb-4">
            <h2 class="text-lg font-semibold">Allergies</h2>
            <p class="text-gray-700">{{ medicalHistory.is_allergic ? 'Yes' : 'No' }}</p>
        </div>

        <div v-if="medicalHistory.is_allergic" class="mb-4">
            <h2 class="text-lg font-semibold">Allergy Details</h2>
            <p class="text-gray-700">{{ medicalHistory.allergies || 'N/A' }}</p>
        </div>

        <div class="mb-4">
            <h2 class="text-lg font-semibold">Family Medical History</h2>
            <p class="text-gray-700">{{ medicalHistory.family_medical_history || 'N/A' }}</p>
        </div>

        <div class="mb-4">
            <h2 class="text-lg font-semibold">Previous Surgeries/Hospitalizations</h2>
            <p class="text-gray-700">{{ medicalHistory.previous_surgeries_or_hospitalizations || 'N/A' }}</p>
        </div>

        <div class="mb-4">
            <h2 class="text-lg font-semibold">Date & Time</h2>
            <p class="text-gray-700">{{ medicalHistory.clients_date_and_time || 'N/A' }}</p>
        </div>
    </div>
